...
def process_user_query(question):
    ...
    You are an expert in solving Insurance queries. The following is the context given to you to answer
        ...
        answer the query as best as you can. Consider the summary and context as ultimate source of truth, if some information is not there, confidently answer about it.
        ...
        Answer:
    """
    prompt_with_summary = stringPrompt.format(context="{context}", summary=summary, question="{question}")

    prompt = PromptTemplate(
        template=prompt_with_summary,
    ...
    response = rag_chain_with_summary.invoke(question)
    return response